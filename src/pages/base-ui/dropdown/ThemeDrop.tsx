...
export const ThemeDrop: React.FC<DropdownProps> = ({ color = '#3e97ff1a', label, closeType }) => {
          ...
          <div className="py-1">
            <Menu.Item as={Fragment}>
              {({ active }: MenuItemRenderProps) => (
                <a
                  href="#"
                  className={`block px-4 py-[10px] text-sm whitespace-nowrap ${active ? 'bg-black bg-opacity-10' : ''}`}
                  onClick={() => {
            ...
            <div className="border-t border-white border-opacity-20 my-[10px]"></div>
            <Menu.Item as={Fragment}>
              {({ active }: MenuItemRenderProps) => (
                <a
                  href="#"
