...
export default function UnderTab() {
  ...
  return (
    <>
      <div className="flex flex-wrap px-4 pt-4 gap-9">
        {(["home", "profile", "message","setting", "disabled"] as Tab[]).map((tab) => (
          <button
            key={tab}
            disabled={tab === "disabled"}
            onClick={() => tab !== "disabled" && setActiveTab(tab)}
            className={`font-bold py-2 text-sm cursor-pointer transition-all duration-300 outline-none focus:outline-none
              ${
                activeTab === tab
                  ? "border-b-2"
                  : tab === "disabled"
