...
import { useFadeIn } from "../../hooks/useFadeIn";
    ...
    <button
        ...
        className={`text-[#202224] text-sm px-3 sm:px-4 py-1 sm:py-2 rounded-full border border-[#979797] font-semibold w-full  whitespace-nowrap
            ${isActive ? 'bg-[#4880FF] text-white shadow-md' : 'hover:bg-[#4880FF] hover:text-white hover:border-[#4880FF]'}
            ${className || ''}`}
        >
        {children}
    </button>
    ...
    const renderCalendarDays = () => {
        ...
        days.push(<div key={`empty-${i}`} className="w-10 h-10 flex items-center justify-center text-gray-400"></div>);
        }
        ...
        days.push(
            ...
            className={`w-9 h-9 rounded-lg flex items-center justify-center text-sm font-medium
                ${isSelected ? 'bg-[#6085FF] text-white' : 'text-[#202224]/50 hover:bg-gray-100'}
    ...
    return (
        <div className="bg-white rounded-2xl shadow-lg w-[280px] sm:w-[387px]">
            ...
            <span className="text-[#202224] text-[15px] font-bold">{monthNames[currentMonth]} {currentYear}</span>
            <div className="flex gap-3">
                <button onClick={goToPreviousMonth} className="p-1 rounded-md hover:bg-gray-100 bg-[#E7E9EE]">
            <ChevronDown className="w-4 h-4 rotate-90 text-[#121212]" />
            </button>
            <button onClick={goToNextMonth} className="p-1 rounded-md hover:bg-gray-100 bg-[#E7E9EE]">
            <ChevronDown className="w-4 h-4 -rotate-90 text-[#121212]" />
            </button>
            </div>
        </div>
        ...
        <p className="text-xs text-gray-500 pt-2 sm:pt-4 pb-4 sm:pb-8  px-5 sm:px-[31px]">*You can choose multiple dates</p>
    ...
    const orderTypes = [
    'Health & Medicine', 'Book & Stationery', 'Services & Industry',
    'Fashion & Beauty', 'Home & Living', 'Electronics',
    'Mobile & Phone', 'Accessories'
    ];

    const OrderTypeFilter: React.FC<OrderTypeFilterProps> = ({ selectedTypes, onSelectTypes }) => {
    const [currentSelectedTypes, setCurrentSelectedTypes] = useState<string[]>(selectedTypes);
    ...
    return (
        ...
        <h3 className="font-bold text-base sm:text-lg text-[#202224] px-3 sm:px-6 pt-3 sm:pt-6">Select Order Type</h3>
        ...
        <p className="text-xs text-gray-500 px-6 pt-2 sm:pt-4 pb-4 sm:pb-8">*You can choose multiple Order types</p>
    ...
    const OrderStatusFilter: React.FC<OrderStatusFilterProps> = ({ selectedStatuses, onSelectStatuses }) => {
    const [currentSelectedStatuses, setCurrentSelectedStatuses] = useState<string[]>(selectedStatuses);
    ...
    return (
        ...
        <h3 className="font-bold text-base sm:text-lg text-[#202224] px-3 sm:px-6 pt-3 sm:pt-6">Select Order Status</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-3 sm:p-6 border-b border-[#979797]/50">
        ...
        <p className="text-xs text-[#434343]/82 py-4 px-6">*You can choose multiple Order statuses</p>
        <div className="text-center pb-3  sm:pb-6">
        <button
    ...
    const matchesType = selectedOrderTypes.length > 0
        ? selectedOrderTypes.includes(order.type)
        : true;

    const matchesStatus = selectedOrderStatuses.length > 0
